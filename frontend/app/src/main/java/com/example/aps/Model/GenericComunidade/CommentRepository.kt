...
// ... (Outros data classes permanecem inalterados) ...
...
class CommentRepository {
    ...
    private val COMMENT_API_URL = "http://srv1070702.hstgr.cloud:5000/api/comments"

    // ⭐️ FUNÇÃO ATUALIZADA: Removido o parâmetro startDate e sua lógica de construção de URL
    suspend fun fetchCommentsByLineId(
        lineId: String,
        limit: Int? = null,
        // startDate: String? = null, <-- REMOVIDO DA ASSINATURA DA FUNÇÃO
        offset: Int? = null
    ): Result<List<Post>> = withContext(Dispatchers.IO) {
        try {
            // Constrói a URL dinamicamente
            val fetchUrlBuilder = StringBuilder("$COMMENT_API_URL?line_id=$lineId")

            // Adiciona LIMIT (para 20, 30, 50 ou 100 posts)
            if (limit != null) {
                fetchUrlBuilder.append("&limit=$limit")
            }
            // Adiciona OFFSET (para paginação futura)
            if (offset != null) {
                fetchUrlBuilder.append("&offset=$offset")
            }

            // ⚠️ LÓGICA DE START_DATE FOI REMOVIDA ⚠️

            val fetchUrl = fetchUrlBuilder.toString()
